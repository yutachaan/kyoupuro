...
int main() {
  ...
  vi index(N); iota(ALL(index), 0); // 0〜N-1

  // Ai * (Aj + Bj) > Aj * (Ai + Bi) となるように安定ソート
  stable_sort(ALL(index), [&](const auto& l, const auto& r) { return A[l] * (A[r] + B[r]) > A[r] * (A[l] + B[l]); });

  fore(i, index) cout << i + 1 << ' ';
  cout << endl;
}
