...
int main() {
  ...
  vl total_sleep(N, 0);
  rep(i, 2, N) {
    if (i % 2 == 0) total_sleep[i] = total_sleep[i - 1] + (A[i] - A[i - 1]);
    else total_sleep[i] = total_sleep[i - 1];
  }
  ...
  while (Q--) {
    ...
    // l, r以上のはじめてのA[i]のインデックスを取得
    int l_i = lower_bound(ALL(A), l) - begin(A);
    int r_i = lower_bound(ALL(A), r) - begin(A);

    // l, rまでに寝た時間の累計を算出
    ll l_total = l_i % 2 == 0 ? total_sleep[l_i] - (A[l_i] - l) : total_sleep[l_i];
    ll r_total = r_i % 2 == 0 ? total_sleep[r_i] - (A[r_i] - r) : total_sleep[r_i];

    ll ans = r_total - l_total;
