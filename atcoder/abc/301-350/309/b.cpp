...
int main() {
  int N; cin >> N;
  vector A(N, vector<char>(N)); rep(i, 0, N) rep(j, 0, N) cin >> A[i][j];

  // 外側以外は同じ
  vector B(N, vector<char>(N, 'a')); rep(i, 1, N - 1) rep(j, 1, N - 1) B[i][j] = A[i][j];

  // 外側をずらす
  rep(j, 0, N - 1) B[0][j + 1] = A[0][j];
  rep(i, 0, N - 1) B[i + 1][N - 1] = A[i][N - 1];
  rrep(j, N, 1) B[N - 1][j - 1] = A[N - 1][j];
  rrep(i, N, 1) B[i - 1][0] = A[i][0];
