...
int main() {
  ...
  vector<vvl> dp(N + 1, vvl(K + 1, vl(D, -1))); // dp[i][j][k]: i番目までのうちj個を選び，総和をDで割った余りがkであるようなときの総和の最大値
  dp[0][0][0] = 0;

  rep(i, 1, N + 1) rep(j, 0, K + 1) rep(k, 0, D) {
    if (dp[i - 1][j][k] == -1) continue;

    if (j != K) chmax(dp[i][j + 1][(k + a[i]) % D], dp[i - 1][j][k] + a[i]); // i番目を選んだ場合
    chmax(dp[i][j][k], dp[i - 1][j][k]); // i番目を選ばなかった場合
  }
