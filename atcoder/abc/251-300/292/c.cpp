...
int main() {
  ...
  ll ans = 0;
  // 例えばN=4のとき，AB=1, CD=3の場合と， AB=3, CD=1の場合とではパターン数が同じため， N/2まで回してそれぞれ2倍すれば良い
  rep(i, 1, N / 2 + 1) {
    int j = N - i;

    ans += SIZE(enum_divisors(i)) * SIZE(enum_divisors(j)) * 2;
  }

  // Nが偶数のときは，N/2のパターン数が余分に数えられているためそれを除く
  if (N % 2 == 0) {
    ans -= SIZE(enum_divisors(N / 2)) * SIZE(enum_divisors(N / 2));
  }
