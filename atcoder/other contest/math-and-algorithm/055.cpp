...
// 2次正方行列
struct Matrix { mint p[2][2] = {{0, 0}, {0, 0}}; };

// 行列の乗算
Matrix Mul(Matrix A, Matrix B) {
  Matrix ret;
  rep(i, 0, 2) rep(j, 0, 2) rep(k, 0, 2) ret.p[i][j] += A.p[i][k] * B.p[k][j];
...
Matrix Power(Matrix A, ll n) {
  Matrix ret; ret.p[0][0] = ret.p[1][1] = 1;
  while (n > 0) {
...
int main() {
  ...
  Matrix A; A.p[0][0] = 2; A.p[0][1] = A.p[1][0] = 1;
  Matrix B = Power(A, N - 1);

  cout << B.p[1][0] + B.p[1][1] << endl;
}
