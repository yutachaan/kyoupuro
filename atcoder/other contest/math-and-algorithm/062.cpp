...
int main() {
  ...
  vi order; order.push_back(1); // 訪問した都市の順番
  map<int, int> mp; mp[1] = 0;  // ある都市が何番目に訪問されたか
  int i = 1;
  while (mp.count(A[i]) == 0 && SIZE(order) <= K) { // サイクルを検出するまで都市を移動
    order.push_back(A[i]);
    mp[A[i]] = SIZE(order) - 1;
    i = A[i];
  }

  // 最後までサイクルを検出しなかった場合，最後に訪問した都市が答え
  // サイクルを検出した場合，サイクル開始から(K-サイクル開始前までに移動した回数)%(サイクルの大きさ)番目に訪問した都市が答え
  if (SIZE(order) == N) cout << order[SIZE(order) - 1] << endl;
  else cout << order[mp[A[i]] + (K - mp[A[i]]) % (SIZE(order) - mp[A[i]])] << endl; // サイクルを検出した場合
}
