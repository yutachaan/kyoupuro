...
int main() {
  int N; cin >> N;
  vector<pii> LR(N); rep(i, 0, N) cin >> LR[i].first >> LR[i].second;

  // 終了時刻をキーとしてソート
  sort(ALL(LR), [](const auto& l, const auto& r) { return l.second < r.second; });

  int ans = 0;
  int last = 0; // 最後に見た映画の終了時刻
  int i = 0;
  while (i < N && last < 86400) {
    // 開始時刻がlastよりも後ならその映画は見れる
    if (LR[i].first >= last) {
      ans++;
      last = LR[i].second;
    }
