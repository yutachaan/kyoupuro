...
int main() {
  ...
  vector<vvl> dp(N + 1, vvl(N + 1, vl(1001))); // dp[i][j][k]: i番目までのうちj枚選んで総和をkとする場合の数
  dp[0][0][0] = 1;

  rep(i, 1, N + 1) rep(j, 0, i + 1) rep(k, 0, 1001) {
    dp[i][j][k] += dp[i - 1][j][k];                                         // i番目を選ばない場合
    if (j != N && k - A[i] >= 0) dp[i][j + 1][k] += dp[i - 1][j][k - A[i]]; // i番目を選ぶ場合
  }
