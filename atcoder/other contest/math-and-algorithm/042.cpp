...
ll solve(int n) {
  ...
  rep(p, 2, n + 1) {
    count_div[p]++;
    for (int q = p * 2; q <= n; q += p) count_div[q]++;
...
int main() {
  ...
  vl count_div(N + 1, 1); // iの約数の個数

  // エラトステネスの派生的な方法でやる (O(NlogN)で間に合う)
  ll ans = count_div[1];
  rep(p, 2, N + 1) {
    for (int q = p; q <= N; q += p) count_div[q]++; // pの倍数は約数にpをもつため，カウントアップ

    ans += p * count_div[p]; // f(p)を計算
  }
